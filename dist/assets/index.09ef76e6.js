...
const logoBlanco = "/assets/logo-blanco-fems-viajando-juntas.bc76852b.png";
const logoColores = "/assets/logo-fems-viajando-juntas.b640175d.png";
const background1 = "/assets/background-1.9912941b.jpg";
const auth$1 = getAuth(app);
auth$1.languageCode = "es";
function loginWithGoogle() {
  const provider = new GoogleAuthProvider();
  return signInWithPopup(auth$1, provider);
}
function registerNewUser(email, password) {
  return createUserWithEmailAndPassword(auth$1, email, password);
}
function verifyEmail() {
  return sendEmailVerification(auth$1.currentUser);
}
function logout() {
  return signOut(auth$1);
}
function loginWithEmail(email, password) {
  return signInWithEmailAndPassword(auth$1, email, password);
}
...
var wa = x$1("Opera"), y = x$1("Trident") || x$1("MSIE"), xa = x$1("Edge"), ya$1 = xa || y, za$1 = x$1("Gecko") && !(-1 != sa().toLowerCase().indexOf("webkit") && !x$1("Edge")) && !(x$1("Trident") || x$1("MSIE")) && !x$1("Edge"), Aa = -1 != sa().toLowerCase().indexOf("webkit") && !x$1("Edge");
function Ba$1() {
  var a = l.document;
...
function ec$1(a, b2) {
  ...
  0 < a.N && (a.L = new Cb(q$2(a.La, a, a.g), a.N));
      ...
      if (!a.J && qc$1(c.h, a) && 3 == c.G) {
        ...
        if (Array.isArray(d) && 3 == d.length) {
          ...
          if (0 == e[0])
            ...
            c.Ba = e[1], 0 < c.Ba - c.T && 37500 > e[2] && c.L && 0 == c.A && !c.v && (c.v = J(q$2(c.cb, c), 6e3));
          if (1 >= uc$1(c.h) && c.ja) {
            try {
              c.ja();
            } catch (m) {
          ...
          if (2 == c.G)
            if ("c" == m[0]) {
              ...
              if (Z2) {
                ...
                if (Da2) {
                  var f = d.h;
                  f.g || -1 == Da2.indexOf("spdy") && -1 == Da2.indexOf("quic") && -1 == Da2.indexOf("h2") || (f.j = f.l, f.g = /* @__PURE__ */ new Set(), f.h && (vc$1(f, f.h), f.h = null));
                }
                if (d.D) {
...
k$1.da = function(a, b2, c, d) {
  ...
  try {
    pd(this), 0 < this.B && ((this.K = qd(this.g)) ? (this.g.timeout = this.B, this.g.ontimeout = q$2(this.qa, this)) : this.A = Ab(this.qa, this.B, this)), this.v = true, this.g.send(a), this.v = false;
  } catch (f) {
    od(this, f);
  }
};
...
function Dd(a, b2, c) {
  ...
  var d = a.l ? q$2(a.l.Ra, a.l, a) : null;
  a: {
    var e = a.i;
    let f = -1;
    for (; ; ) {
...
class Q {
  ...
  start(t2, e) {
    ...
    let i = new q$1();
    this.o = () => {
      this.i++, this.currentUser = this.u(), i.resolve(), i = new q$1(), t2.enqueueRetryable(() => s(this.currentUser));
    };
    const r = () => {
      const e2 = i;
    ...
    this.t.onInit((t3) => o(t3)), setTimeout(() => {
      if (!this.auth) {
        ...
        t3 ? o(t3) : (C("FirebaseAuthCredentialsProvider", "Auth not yet detected"), i.resolve(), i = new q$1());
      }
    }, 0), r();
  }
  getToken() {
...
class oc {
  constructor(t2, e, n, s, i) {
    this.asyncQueue = t2, this.timerId = e, this.targetTimeMs = n, this.op = s, this.removalCallback = i, this.deferred = new q$1(), this.then = this.deferred.promise.then.bind(this.deferred.promise), this.deferred.promise.catch((t3) => {
    });
  }
...
function Kl(t2) {
  t2 = Ua(t2, za);
  const e = Ua(t2.firestore, rh);
  return Da(ch(e), t2._key).then((n) => sf(e, t2, n));
}
class Gl extends Sl {
  constructor(t2) {
...
function Jl(t2, e, n) {
  t2 = Ua(t2, za);
  const s = Ua(t2.firestore, rh), i = Dl(t2.converter, e, n);
  return nf(s, [Oh(kh(s), "setDoc", t2._key, i, null !== t2.converter, n).toMutation(t2._key, Hn.none())]);
}
...
function tf(t2, ...e) {
  ...
  let c, a, h;
  if (t2 instanceof za)
    a = Ua(t2.firestore, rh), h = wn(t2._key.path), c = {
      next: (n2) => {
        e[o] && e[o](sf(a, t2, n2));
      },
      error: e[o + 1],
    ...
    const n2 = Ua(t2, Wa);
    a = Ua(n2.firestore, rh), h = n2._query;
    const s2 = new Gl(a);
    c = {
      next: (t3) => {
        e[o] && e[o](new Ll(a, s2, n2, t3));
      },
      error: e[o + 1],
      complete: e[o + 2]
    }, sl(t2._query);
  ...
  return function(t3, e2, n2, s2) {
    const i2 = new fa(s2), r2 = new yc(e2, i2, n2);
    return t3.asyncQueue.enqueueAndForget(async () => dc(await va(t3), r2)), () => {
      i2.Pc(), t3.asyncQueue.enqueueAndForget(async () => _c(await va(t3), r2));
    };
  }(ch(a), h, u, c);
}
function nf(t2, e) {
...
function sf(t2, e, n) {
  const s = n.docs.get(e._key), i = new Gl(t2);
  return new $l(t2, i, e._key, s, new Fl(n.hasPendingWrites, n.fromCache), e.converter);
}
function _f(...t2) {
...
const db = uh(app);
const collectionPost = Ja(db, "posts");
const q = ol(collectionPost, dl("createdAt", "desc"));
const savePost = (post) => {
  const userId = auth$1.currentUser;
  Xl(collectionPost, {
    post,
    userUid: userId.uid,
...
const addLikePost = (id2, uidCurrentUser) => {
  Yl(Za(db, "posts", id2), { like: _f(uidCurrentUser) });
};
const removeLikePost = (id2, uidCurrentUser) => {
  Yl(Za(db, "posts", id2), { like: wf(uidCurrentUser) });
};
const onGetPosts = (callback) => tf(q, callback);
const deletePost = (id2) => Zl(Za(db, "posts", id2));
const getPost = (id2) => Kl(Za(db, "posts", id2));
const updatePost = (id2, newFields) => Yl(Za(db, "posts", id2), newFields);
const saveUser = (user, fullName, userName) => {
  Jl(Za(db, "users", user.uid), {
    id: user.uid,
    email: user.email,
    name: fullName,
    userUser: userName
...
const Welcome = (onNavigate2) => {
  ...
  descriptionContentModal.textContent = "Fems es un espacio seguro solo para mujeres donde podr\xE1s compartir experiencias e informaci\xF3n sobre viajes.";
...
const Login = (onNavigate2) => {
  ...
  title.textContent = "Iniciar sesi\xF3n";
  labelEmail.textContent = "E-mail";
  errorEmail.textContent = "El email debe tener un formato valido.";
  labelPassword.textContent = "Contrase\xF1a";
  errorPassword.textContent = "La contrase\xF1a debe tener de 8 a 16 d\xEDgitos, may\xFAscula, min\xFAscula, n\xFAmero, caracter especial y no acepta espacios.";
  buttonLogin.textContent = "Iniciar sesi\xF3n";
  ...
  function userLogin() {
    ...
    loginWithEmail(email, password).then((result) => {
      ...
      if (user.emailVerified === false) {
        alert("Email no verificado, se le envi\xF3 un correo de verificaci\xF3n");
      } else {
        onNavigate2("/home");
      }
    }).catch((error) => {
...
const background2 = "/assets/background-2.74171009.png";
const Signup = (onNavigate2) => {
  const form = document.createElement("form");
  const title = document.createElement("h2");
  const divContent = document.createElement("div");
  ...
  errorUser.textContent = "El usuario tiene que ser de 4 a 16 d\xEDgitos y solo puede contener numeros, letras y guion bajo.";
  labelName.textContent = "Nombre y Apellido";
  errorName.textContent = "El usuario tiene que ser de 4 a 16 d\xEDgitos y solo puede contener letras. ";
  labelEmail.textContent = "E-mail";
  errorEmail.textContent = "El email debe tener un formato valido. ";
  labelPassword.textContent = "Contrase\xF1a";
  errorPassword.textContent = "La contrase\xF1a debe tener de 8 a 16 d\xEDgitos, may\xFAscula, min\xFAscula, n\xFAmero, caracter especial y no acepta espacios.";
  labelPassword2.textContent = "Confirmar contrase\xF1a";
  errorPassword2.textContent = "Ambas contrase\xF1as deben ser iguales.";
  correctMessage.textContent = "Formulario enviado, verifica tu email e inicia sesi\xF3n.";
  errorMessage.textContent = "Por favor verifica que tus datos sean v\xE1lidos.";
...
const Home = (onNavigate2) => {
  ...
  onGetPosts((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const currentUserUid = auth$1.currentUser.uid;
  ...
  `;
    ...
    btnLikes.forEach((btnLike) => {
      btnLike.addEventListener("click", () => {
        ...
        const userUid = auth$1.currentUser.uid;
        getPost(likedButton).then((doclike) => {
    ...
    btnsDelete.forEach((btn) => {
      btn.addEventListener("click", ({ target: { dataset } }) => {
        const confirmDelete = confirm("\xBFSegura que deseas eliminar este post?");
        if (confirmDelete) {
