...
class il extends nl {
  ...
  _apply(t2) {
    ...
    return Al(t2._query, e), new pa(t2.firestore, t2.converter, yn(t2._query, e));
  }
  _parse(t2) {
...
class ol extends el {
  ...
  _apply(t2) {
    ...
    return 0 === e.getFilters().length ? t2 : (function(t3, e2) {
      ...
      for (const t4 of s)
        Al(n, t4), n = yn(n, t4);
    }(t2._query, e), new pa(t2.firestore, t2.converter, yn(t2._query, e)));
  }
...
!function(t2, e = true) {
  ...
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(b, "3.8.1", t2), registerVersion(b, "3.8.1", "esm2017");
}();
const db = oh(app);
const collectionPost = Ta(db, "posts");
const q = sl(collectionPost, hl("createdAt", "desc"));
const savePost = (post) => {
...
const addLikePost = (id2, uidCurrentUser) => {
  Kl(Aa(db, "posts", id2), { like: of(uidCurrentUser) });
};
const removeLikePost = (id2, uidCurrentUser) => {
  Kl(Aa(db, "posts", id2), { like: uf(uidCurrentUser) });
};
const onGetPosts = (callback) => jl(q, callback);
const deletePost = (id2) => Gl(Aa(db, "posts", id2));
const getPost = (id2) => Ol(Aa(db, "posts", id2));
const updatePost = (id2, newFields) => Kl(Aa(db, "posts", id2), newFields);
const saveUser = (user, fullName, userName) => {
  Ul(Aa(db, "users", user.uid), {
    id: user.uid,
    email: user.email,
    name: fullName,
    userUser: userName
